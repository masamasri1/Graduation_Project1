...
import 'package:flutter_responsive/wotp.dart';
import 'connection/connect.dart';
import 'package:flutter_responsive/adminlogin.dart';
import 'package:http/http.dart' as http;
import 'package:fluttertoast/fluttertoast.dart';

class WorkerForgetpass extends StatefulWidget {
  const WorkerForgetpass({super.key});

  @override
  State<WorkerForgetpass> createState() => _WorkerForgetpass();
}

class _WorkerForgetpass extends State<WorkerForgetpass> {
  TextEditingController fpass = TextEditingController();
  Crud _crud = Crud();
  Future _resetPassword() async {
    var response = await _crud.postRequest(applink.workerforgetpass, {
      'craftmen_gmail': fpass.text,
    });
    if (response['status'] == "success") {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => workerotp(email: fpass.text)));
    } else {
