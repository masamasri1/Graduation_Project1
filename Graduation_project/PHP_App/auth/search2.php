...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Check if 'emergency' and 'job_id' are set in the POST parameters
    if (isset($_POST['emergency']) && isset($_POST['jop_id'])) {
        // Get the search query from the POST parameters
        $emergency = '%' . $_POST['emergency'] . '%';
        $job_id = $_POST['jop_id'];

        // Prepare the SQL statement with a WHERE clause for search
        $sql = "SELECT * FROM craftmen WHERE LOWER(emergency) LIKE LOWER(?) AND jop_id = ?";
        ...
        $stmt->bind_param("si", $emergency, $job_id);
        $stmt->execute();
        ...
        if ($result->num_rows > 0) {
            ...
            // No rows found
            echo json_encode(array("status" => "fail", "message" => "No data found for emergency: $emergency and jop_id: $job_id"));
        }
        ...
        // 'emergency' or 'job_id' not set in the POST parameters
        echo json_encode(array("status" => "fail", "message" => "'emergency' and 'jop_id' parameters are required"));
    }
} else {
