...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $adminEmail = $_POST['admin_gmail'];

    $decodedPhoto = base64_decode($base64Photo);

    if (!empty($decodedPhoto)) {
        $stmt = $con->prepare("UPDATE admin SET aphoto = ? WHERE admin_gmail = ?");
        ...
        if ($stmt->execute()) {
            ...
            $selectStmt = $con->prepare("SELECT aphoto FROM admin WHERE admin_gmail = ?");
